...
namespace Apteka
{
	public partial class MedicineProductsForm : FormWithNotification
	{
		...
		public MedicineProductsForm()
		{
			InitializeComponent();
			_viewModel = new();
			_viewModel.ConfigureSettingsDGVMedicineProduct(dgvMedicineProduct);
			_viewModel.SetDefaultDataSourceMedicineProduct(dgvMedicineProduct);
			_viewModel.ConfigureSettingsDGVMedicineProductCost(dgvMedicineProductCost);
		...
		{
			_viewModel.General.DatabaseNotificationService.Subscribe("medicine_product",
				data =>
				{
			...
			_viewModel.General.DatabaseNotificationService.Subscribe("medicine_cost",
				data =>
				{
		...
		{
			dgvMedicineProduct.Columns["IdMedicineProduct"].Visible =
			dgvMedicineProduct.Columns["IdMedicine"].Visible =
		...
		private string[] GetStrParameters(bool isForExtendedParameters)
		{
			if (isForExtendedParameters)
				...
				return [TextSearch, SerialNumber, MedicineForm, WayEnter,
								PackagingForm, Producer, PharmGroup, StorageCondition,
		...
		{
			bool isAnythingFind = await _viewModel.SearchMedicineProductAsync(
				dgvMedicineProduct, IdMedicine,
			GetStrParameters(false), GetDoParameters(),
		...
		internal void SearchMedicineProductFromStorageMedicineProductsForm(string serialNumber)
		{
...
#pragma warning restore CS8602 // Разыменование вероятной пустой ссылки.
		...
		{
			if (dgvMedicineProduct.CurrentRow != null && !dgvMedicineProduct.CurrentRow.IsNewRow)
			{
				DataGridViewRow selectedRow = dgvMedicineProduct.CurrentRow;
